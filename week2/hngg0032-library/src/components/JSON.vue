...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="austenWorks in austen?.famousWorks" :key="austenWorks.title">
          {{ austenWorks.title }} ({{ austenWorks.year }})
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(hours, day) in bookstores.openingHours" :key="day">
          For {{ day }}, the opening hours are from {{ hours.open }} to {{ hours.close }}.
        </li>
      </ul>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="getHighlightClass(author)"
          :style="getHighlightStyle(author)"
          :title="author.name"
        >
          {{ author.name }} {{ author.birthYear }}
        </li>
...
// const orwell = computed(() => authors.filter((author) => author.name.includes('Orwell')))
...
const getHighlightClass = (author) => {
  return { highlight: isHighlighted(author) }
}

const getHighlightStyle = (author) => {
  return {
    color: isHighlighted(author) ? 'purple' : 'black',
    fontWeight: isHighlighted(author) ? 'bold' : 'normal',
    backgroundColor: isHighlighted(author) ? 'lightyellow' : 'transparent'
  }
}
</script>
